...
const EOD: React.FC<EODProps> = ({ user, onUpdateUser }) => {
    ...
    const eodHistoryMap = useMemo(() => {
        const map = new Map<string, StoreEODEntry>();
        eodHistory.forEach(e => map.set(e.date, e));
        return map;
    }, [eodHistory]);
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {viewMode === 'entry' ? (
                ...
                <GlassCard className="p-4 space-y-6 rounded-3xl animate-reveal overflow-hidden"><div className="flex items-center justify-between px-2"><button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} className="p-2 rounded-3xl hover:bg-white/40 dark:hover:bg-white/10 transition-colors"><ChevronLeft size={20} /></button><h3 className="font-bold text-lg">{currentMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}</h3><button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1))} className="p-2 rounded-3xl hover:bg-white/40 dark:hover:bg-white/10 transition-colors"><ChevronRight size={20} /></button></div><div className="grid grid-cols-7 gap-2">{['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => ( <div key={i} className="text-center text-[10px] font-bold text-zinc-400 uppercase opacity-50">{d}</div> ))}{(() => { const todayString = new Date().toISOString().split('T')[0]; return calendarDays.map((d, i) => { if (!d) return <div key={i} />; const entry = eodHistoryMap.get(d.dateStr); const isToday = d.dateStr === todayString; const isAchieved = entry && entry.achievement >= entry.dayTarget; return ( <div key={d.dateStr} onClick={() => { setDate(d.dateStr); setViewMode('entry'); }} className={`aspect-square rounded-3xl border flex flex-col items-center justify-center cursor-pointer transition-all duration-300 relative ${isToday ? 'border-blue-500 ring-1 ring-blue-200 dark:ring-blue-900/50 scale-105 z-10' : 'border-white/40 dark:border-white/10 hover:bg-white/40 dark:hover:bg-white/10 hover:scale-110'} ${entry ? 'bg-white/60 dark:bg-zinc-800/80 backdrop-blur-md shadow-sm border-white/50 dark:border-white/20' : 'bg-transparent'}`}> <span className={`text-xs ${isToday ? 'font-black text-blue-600' : 'text-zinc-500'}`}>{d.day}</span> {entry && ( <div className={`w-1.5 h-1.5 rounded-3xl mt-1 ${isAchieved ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-blue-400'}`} /> )} </div> ); }); })()}</div><div className="space-y-3 pt-4 border-t border-gray-200/50 dark:border-white/10"><h4 className="text-xs font-bold text-zinc-400 uppercase tracking-widest px-1">Weekly History</h4><div className="space-y-3">{eodHistory.filter(e => e.date.startsWith(currentMonth.toISOString().slice(0, 7))).sort((a,b) => b.date.localeCompare(a.date)).map((entry, idx) => ( <div key={entry.date} className="flex justify-between items-center p-3 bg-white/50 dark:bg-white/5 backdrop-blur-md rounded-3xl border border-white/40 dark:border-white/20 hover:border-blue-400/50 transition-all group"> <div><p className="text-sm font-bold">{formatToDisplayDate(entry.date)}</p><p className="text-[10px] text-zinc-500">Achieved: ₹{entry.achievement.toLocaleString()}</p></div><div className="flex items-center gap-4"><div className={`px-2 py-0.5 rounded-3xl border text-[10px] font-bold ${entry.achievement >= entry.dayTarget ? 'bg-green-100/50 border-green-200 text-green-700 dark:text-green-400' : 'bg-blue-100/50 border-blue-200 text-blue-700 dark:text-blue-400'}`}>{Math.round((entry.achievement / (entry.dayTarget || 1)) * 100)}%</div><button onClick={(e) => { e.stopPropagation(); handleDelete(entry.date); }} className="text-red-500 hover:text-red-600 p-1 hover:bg-red-100/50 dark:hover:bg-red-900/30 rounded-3xl transition-colors"><Trash2 size={16} /></button></div></div> ))}</div></div></GlassCard>
            )}
