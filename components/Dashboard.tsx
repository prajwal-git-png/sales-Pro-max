  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard className="p-5 animate-in slide-in-from-bottom-4 duration-500 rounded-3xl">
          ...
          <div className="h-3 w-full bg-zinc-200/50 dark:bg-zinc-800/50 rounded-full overflow-hidden mt-1 shadow-inner"><div className="h-full bg-zinc-900 dark:bg-zinc-100 transition-all duration-1000 ease-out" style={{ width: `${mtdPercentage}%` }}></div></div>
      ...
      <div className="flex gap-2"><GlassButton onClick={handleCopyToday} className="flex-1 !py-3 bg-white/60 dark:bg-zinc-800/60 backdrop-blur-md hover:bg-white/80 dark:hover:bg-zinc-700/80 !text-sm border border-white/50 dark:border-white/10 rounded-2xl text-zinc-900 dark:text-white"><Copy size={16} /> Copy Report</GlassButton><GlassButton onClick={() => setShowEODModal(true)} className="flex-1 !py-3 bg-zinc-900 dark:bg-zinc-100 hover:bg-zinc-800 dark:hover:bg-white text-white dark:text-zinc-900 !text-sm rounded-2xl border-none"><Store size={16} /> Store EOD</GlassButton></div>
      <div className="flex p-1 bg-white/40 dark:bg-zinc-900/40 backdrop-blur-md rounded-full w-fit mx-auto shadow-sm border border-white/50 dark:border-white/10"><button onClick={() => setViewMode('calendar')} className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${viewMode === 'calendar' ? 'bg-white dark:bg-zinc-800 shadow-sm text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400'}`}>Calendar</button><button onClick={() => setViewMode('list')} className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${viewMode === 'list' ? 'bg-white dark:bg-zinc-800 shadow-sm text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400'}`}>List View</button></div>
