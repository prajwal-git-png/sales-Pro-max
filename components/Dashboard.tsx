...
const Dashboard: React.FC<DashboardProps> = ({ sales, user, onDataChange, onUpdateUser }) => {
  ...
  return (
    <div className="space-y-6">
      <GlassCard className="p-4 bg-gradient-to-r from-zinc-50 to-white dark:from-zinc-900 dark:to-zinc-800 animate-in slide-in-from-top-4 duration-500 border-l-4 border-l-yellow-400 shadow-lg rounded-none">
         <div className="flex gap-3">
             <div className="bg-yellow-400/20 p-2 rounded-none h-fit"><Quote className="text-yellow-600 dark:text-yellow-400" size={18} fill="currentColor" fillOpacity={0.5} /></div>
      ...
      <div className="grid grid-cols-3 gap-2">
          <GlassCard className="p-3 flex flex-col items-center justify-center text-center bg-blue-50/50 dark:bg-blue-900/10 animate-in zoom-in duration-500 delay-100 border-blue-100/50 dark:border-blue-500/20 rounded-none"><Target size={20} className="text-blue-500 mb-1" /><p className="text-[10px] uppercase text-zinc-500 font-bold">Target</p><p className="text-sm font-bold truncate w-full"><CountUp end={user.monthlyTarget / 1000} prefix="₹" suffix="k" /></p></GlassCard>
          <GlassCard className="p-3 flex flex-col items-center justify-center text-center bg-green-50/50 dark:bg-green-900/10 animate-in zoom-in duration-500 delay-200 border-green-100/50 dark:border-green-500/20 rounded-none"><Trophy size={20} className="text-green-500 mb-1" /><p className="text-[10px] uppercase text-zinc-500 font-bold">Achieved</p><p className="text-sm font-bold truncate w-full"><CountUp end={mtdValue} prefix="₹" /></p></GlassCard>
          <GlassCard className="p-3 flex flex-col items-center justify-center text-center bg-orange-50/50 dark:bg-orange-900/10 animate-in zoom-in duration-500 delay-300 border-orange-100/50 dark:border-orange-500/20 rounded-none"><Wallet size={20} className="text-orange-500 mb-1" /><p className="text-[10px] uppercase text-zinc-500 font-bold">Balance</p><p className="text-sm font-bold truncate w-full"><CountUp end={balance} prefix="₹" /></p></GlassCard>
      </div>

      <GlassCard className="p-5 animate-in slide-in-from-bottom-4 duration-500 rounded-none">
          ...
          <div className="h-4 w-full bg-zinc-200/50 dark:bg-zinc-700/50 rounded-none overflow-hidden shadow-inner border border-white/30 dark:border-white/10"><div className="h-full bg-zinc-800 dark:bg-zinc-200 transition-all duration-1000 ease-out relative" style={{ width: `${mtdPercentage}%` }}><div className="absolute inset-0 bg-white/20 animate-[shine_2s_infinite]" /></div></div>
      ...
      <div className="flex gap-2"><GlassButton onClick={handleCopyToday} className="flex-1 !py-3 bg-white/50 dark:bg-zinc-800/60 backdrop-blur-md hover:bg-white/70 dark:hover:bg-zinc-700/80 !text-sm border border-white/40 dark:border-white/20 rounded-none"><Copy size={16} /> Copy Report</GlassButton><GlassButton onClick={() => setShowEODModal(true)} className="flex-1 !py-3 bg-blue-600/80 hover:bg-blue-600/90 backdrop-blur-md !text-sm !border-blue-400/30 shadow-sm rounded-none"><Store size={16} /> Store EOD</GlassButton></div>
      <div className="flex p-1 bg-white/40 dark:bg-white/10 rounded-none border border-white/50 dark:border-white/20 backdrop-blur-md w-fit mx-auto"><button onClick={() => setViewMode('calendar')} className={`px-6 py-2 rounded-none text-sm font-medium transition-all ${viewMode === 'calendar' ? 'bg-white/80 dark:bg-zinc-800/80 shadow-sm text-black dark:text-white' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400'}`}>Calendar</button><button onClick={() => setViewMode('list')} className={`px-6 py-2 rounded-none text-sm font-medium transition-all ${viewMode === 'list' ? 'bg-white/80 dark:bg-zinc-800/80 shadow-sm text-black dark:text-white' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400'}`}>List View</button></div>

      {viewMode === 'calendar' ? (
        <GlassCard className="p-4 rounded-none">
          <div className="flex items-center justify-between mb-4 px-2"><button onClick={prevMonth} className="p-2 rounded-none hover:bg-zinc-100 dark:hover:bg-white/10 transition-colors"><ChevronLeft size={20} className="text-zinc-600 dark:text-zinc-300" /></button><h3 className="text-lg font-bold text-zinc-800 dark:text-white flex items-center gap-2"><CalendarIcon size={18} className="text-blue-500" />{currentMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}</h3><button onClick={nextMonth} className="p-2 rounded-none hover:bg-zinc-100 dark:hover:bg-white/10 transition-colors"><ChevronRight size={20} className="text-zinc-600 dark:text-zinc-300" /></button></div>
        ...
        <div className="space-y-4">
          <div className="px-2">
            ...
            {showFilters && (
                <GlassCard className="mt-2 p-4 animate-in slide-in-from-top-2 rounded-none">
                    <div className="space-y-3"><div className="relative"><SearchIcon size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" /><input className="w-full bg-white/50 dark:bg-black/40 border border-white/50 dark:border-white/20 rounded-none pl-9 pr-3 py-2 text-sm outline-none backdrop-blur-md text-zinc-800 dark:text-white placeholder-zinc-500" placeholder="Filter by product name..." value={filters.productName} onChange={e => setFilters({...filters, productName: e.target.value})} /></div><div className="grid grid-cols-3 gap-3"><input type="date" className="bg-white/50 dark:bg-black/40 border border-white/50 dark:border-white/20 rounded-none px-3 py-2 text-sm outline-none backdrop-blur-md text-zinc-800 dark:text-white" value={filters.date} onChange={e => setFilters({...filters, date: e.target.value})} /><input type="number" placeholder="Min Qty" className="bg-white/50 dark:bg-black/40 border border-white/50 dark:border-white/20 rounded-none px-3 py-2 text-sm outline-none backdrop-blur-md text-zinc-800 dark:text-white placeholder-zinc-500" value={filters.minQty} onChange={e => setFilters({...filters, minQty: e.target.value})} /><input type="number" placeholder="Min Price" className="bg-white/50 dark:bg-black/40 border border-white/50 dark:border-white/20 rounded-none px-3 py-2 text-sm outline-none backdrop-blur-md text-zinc-800 dark:text-white placeholder-zinc-500" value={filters.minPrice} onChange={e => setFilters({...filters, minPrice: e.target.value})} /></div><div className="flex justify-end pt-2"><button onClick={() => setFilters({ productName: '', date: '', minQty: '', minPrice: '' })} className="text-xs text-red-500 font-bold hover:underline">Clear All</button></div></div>
                </GlassCard>
            )}
          </div>
          {[...filteredSales].map((report, i) => (
              <GlassCard key={report.date} className="p-4 animate-in slide-in-from-bottom-4 duration-500 rounded-none" onClick={() => setSelectedDateReport(report)} style={{ animationDelay: `${i * 0.05}s` }}>
                <div className="flex justify-between items-center"><div><p className="font-bold flex items-center gap-2 text-zinc-900 dark:text-zinc-100">{formatToDisplayDate(report.date)}{report.isWeekOff && <span className="text-[10px] bg-zinc-200/80 dark:bg-zinc-800/80 text-zinc-600 dark:text-zinc-300 px-2 py-0.5 rounded-none border border-zinc-300 dark:border-zinc-600">OFF</span>}</p><p className="text-sm text-zinc-500">{report.isWeekOff ? 'Week Off' : `${report.totalQty} items • ₹${report.totalValue.toLocaleString()}`}</p></div><div className="h-8 w-8 bg-white/50 dark:bg-zinc-800/50 border border-white/40 dark:border-white/20 rounded-none flex items-center justify-center text-black dark:text-white"><List size={16} /></div></div>
             </GlassCard>
          ))}
      ...
      <Modal isOpen={!!selectedDateReport} onClose={() => { setSelectedDateReport(null); cancelEdit(); }} title={selectedDateReport ? formatToDisplayDate(selectedDateReport.date) : ''}>
        {selectedDateReport && (
          <div className="space-y-6 pb-4">
            {selectedDateReport.isWeekOff ? ( <div className="text-center py-8 bg-white/40 dark:bg-white/5 rounded-none border border-dashed border-white/50 dark:border-white/20 backdrop-blur-sm"><Ban size={40} className="mx-auto text-zinc-400 mb-2" /><p className="text-zinc-500 font-medium">Marked as Week Off</p></div> ) : (
                <>
                <div className="grid grid-cols-2 gap-4"><div className="bg-blue-50/50 dark:bg-blue-900/30 border border-blue-100/50 dark:border-blue-500/20 backdrop-blur-sm p-3 rounded-none text-center"><p className="text-xs text-zinc-500">Value</p><p className="font-bold text-blue-600">₹{selectedDateReport.totalValue.toLocaleString()}</p></div><div className="bg-purple-50/50 dark:bg-purple-900/30 border border-purple-100/50 dark:border-purple-500/20 backdrop-blur-sm p-3 rounded-none text-center"><p className="text-xs text-zinc-500">Quantity</p><p className="font-bold text-purple-600">{selectedDateReport.totalQty}</p></div></div>
                <div className="space-y-3"><h4 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider sticky top-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl py-2 z-10">Items Sold</h4>
                    {selectedDateReport.items.map((item, idx) => {
                        ...
                        return (
                        <div key={idx} className={`flex justify-between items-center p-3 rounded-none border transition-all ${isEditing ? 'bg-blue-50/50 dark:bg-blue-900/20 border-blue-200/50 dark:border-blue-500/20' : 'bg-white/40 dark:bg-white/5 border-white/40 dark:border-white/10 backdrop-blur-sm'}`}>
                            {isEditing && editItemState ? (
                                <div className="w-full space-y-2"><input className="w-full bg-transparent border-b border-blue-300 dark:border-blue-500/50 text-sm font-medium focus:outline-none text-zinc-800 dark:text-white" value={editItemState.productName} onChange={(e) => setEditItemState({...editItemState, productName: e.target.value})} placeholder="Product Name" /><div className="flex gap-2"><div className="flex-1"><label className="text-[9px] uppercase text-zinc-500 font-bold">Qty</label><input type="number" className="w-full bg-transparent border-b border-blue-300 dark:border-blue-500/50 text-sm focus:outline-none text-zinc-800 dark:text-white" value={editItemState.quantity} onChange={(e) => setEditItemState({...editItemState, quantity: parseInt(e.target.value) || 0})} /></div><div className="flex-1"><label className="text-[9px] uppercase text-zinc-500 font-bold">Unit Price</label><input type="number" className="w-full bg-transparent border-b border-blue-300 dark:border-blue-500/50 text-sm focus:outline-none text-zinc-800 dark:text-white" value={editItemState.price} onChange={(e) => setEditItemState({...editItemState, price: parseFloat(e.target.value) || 0})} /></div></div><div className="flex justify-end gap-2 mt-2"><button onClick={cancelEdit} className="p-1 rounded-none bg-zinc-200/80 dark:bg-zinc-700/80 text-zinc-600 dark:text-zinc-300"><X size={14} /></button><button onClick={() => saveEditItem(selectedDateReport, idx)} className="p-1 rounded-none bg-green-500/80 text-white"><Check size={14} /></button></div></div>
                            ) : (
                                <><div><p className="font-medium text-sm text-zinc-800 dark:text-white">{item.productName}</p><p className="text-xs text-zinc-500">₹{item.price} x {item.quantity}</p></div><div className="flex items-center gap-2"><p className="font-bold text-sm mr-2 text-zinc-800 dark:text-white">₹{item.price * item.quantity}</p><button onClick={() => startEditItem(item, idx)} className="text-blue-500 hover:text-blue-600 p-1"><Pencil size={14} /></button><button onClick={() => handleRemoveItem(selectedDateReport, idx)} className="text-red-500 hover:text-red-600 p-1"><Trash2 size={14} /></button></div></>
                            )}
                        </div>
                    )})}
                </div>
                {getReportImages(selectedDateReport).length > 0 && (
                    <div className="mt-4"><h4 className="text-sm font-semibold text-zinc-500 mb-2 sticky top-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl py-2 z-10">Bill Copies ({getReportImages(selectedDateReport).length})</h4><div className="grid grid-cols-2 gap-3">
                            {getReportImages(selectedDateReport).map((img, idx) => (
                                <div key={idx} className="report-img-inner-shadow relative group rounded-none overflow-hidden border border-white/40 dark:border-white/20 aspect-square shadow-sm"><img src={img} alt="Bill" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity gap-2 z-10"><div className="flex gap-2"><button onClick={() => setZoomedImage(img)} className="p-2 bg-white/80 backdrop-blur-md rounded-none text-black hover:bg-white"><Maximize2 size={16} /></button><button onClick={() => downloadImage(img, selectedDateReport.date, idx)} className="p-2 bg-white/80 backdrop-blur-md rounded-none text-black hover:bg-white"><Download size={16} /></button></div><button onClick={() => handleRemoveImage(selectedDateReport, idx)} className="px-3 py-1 bg-red-500/80 backdrop-blur-md text-white text-xs rounded-none hover:bg-red-600 mt-2">Delete</button></div></div>
                            ))}
                        </div></div>
                )}
                </>
