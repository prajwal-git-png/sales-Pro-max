  ...
  return (
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-5 fade-in duration-500">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <GlassCard className="p-5 rounded-none">
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                    <label className="cursor-pointer bg-white/60 dark:bg-zinc-800/60 backdrop-blur-md border border-white/50 dark:border-white/20 text-zinc-600 dark:text-zinc-300 px-3 py-1.5 rounded-none text-xs font-bold uppercase tracking-wide flex items-center gap-1.5 hover:bg-white/80 dark:hover:bg-zinc-700/80 transition-colors shadow-sm">
                        <Camera size={14} /> Add
        ...
        {!isWeekOff && (
        <div className="bg-white/60 dark:bg-blue-900/40 backdrop-blur-xl border border-white/50 dark:border-blue-500/30 rounded-none p-4 flex justify-between items-center shadow-sm">
        ...
        <GlassButton type="submit" className="w-full py-4 text-lg shadow-xl shadow-blue-500/20 rounded-none" disabled={isSubmitting}>
    ...
    <Modal isOpen={showSuccessModal} onClose={resetForm} title={isWeekOff ? "Week Off Recorded" : "Entry Saved!"}>
        <div className="text-center space-y-6 pt-4">
            ...
            {!isWeekOff && (
            <div className="grid grid-cols-2 gap-4">
                <button onClick={handleShareWhatsApp} className="flex flex-col items-center justify-center gap-2 p-4 rounded-none bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 transition-all border border-[#25D366]/20 hover:scale-[1.02]"><Send size={24} /><span className="font-bold text-sm">WhatsApp</span></button>
                <button onClick={handleCopyText} className="flex flex-col items-center justify-center gap-2 p-4 rounded-none bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 transition-all border border-blue-500/20 hover:scale-[1.02]"><Copy size={24} /><span className="font-bold text-sm">Copy</span></button>
            </div>
            )}
            <GlassButton onClick={resetForm} variant="secondary" className="w-full rounded-none">Close & New Entry</GlassButton>
        </div>
    </Modal>
    </>
  );
